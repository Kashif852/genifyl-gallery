...
const Tab2Grid = ({ images, handleImageClick, theme }) => {
  ...
  useEffect(() => {
    ...
    if (images && images.length > 0) {
      handleImageClick(images[0].prompt);
    }
  }, [theme, images]);

  const handleImageGrid = (prompt, index) => {
    handleImageClick(prompt);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    if (images.length > 0) {
      ...
      handleImageClick(images[newStartIndex].prompt);
    }
  };
      ...
      <Row style={{ marginTop: "3rem" }}>
        {currentImages.map((image, index) => (
          <Col key={index} xs={12} sm={6} md={4} lg={3} className="mb-4">
            <Card
              onClick={() => handleImageGrid(image.prompt, index)}
              className={`${styles.imageCard} ${
                selectedIndex === (startIndex + index) ? styles.selectedCard : ""
              }`}
            >
              <Card.Img
                style={{ height: "auto" }}
